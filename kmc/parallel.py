...
class ParallelMC(object):
    ...
    def run(self, steps, repeats, run_cls, smpl_classes, **kwargs):
        """ This call runs the KMC in parallel mode """
        # ------------------------
        ...
        # -------------------------------
        results = self.run_thread(steps, n_repeats, run_cls, smpl_classes, **kwargs)

        # Gathering the results (which is the dictionary)
        results_collected = comm.gather(results, root=0)
        self.process_results(results_collected, steps, repeats, smpl_classes, **kwargs)


    def run_thread(self, steps, n_repeats, run_cls, smpl_classes, **kwargs):
        start_time = time.time()
        results = []
        for repeat in xrange(int(n_repeats)):
            results.append(run_cls().run(steps, smpl_classes, **kwargs))
        delta = time.time() - start_time
        return results


    def process_results(self, results, steps, repeats, smpl_classes, **kwargs):
        ...
        if rank == 0:
            ...
            for i, sampler_class in enumerate(smpl_classes):
                ...
                sampler_class.merge(merged, steps, repeats, **kwargs)
